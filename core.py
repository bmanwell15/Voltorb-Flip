...
from markerTiles import Marker
...
#icon = pyglet.image.load("Assets/Images/Icon.png")
...
VERSIONINFO = pyglet.text.Label("V. " + VERSION, x=window.width - 100, y = 5, width = 95, font_size = 10, color = (0,0,0,255), align="right", batch=drawSprites)
...
markerStatus = "clear"
markerList = ["clear", "volt", "one"]

markers = {
    "clear": Marker("clear", window.width, drawSprites),
    "volt": Marker("volt", window.width, drawSprites),
    "one": Marker("one", window.width, drawSprites),
}

markers["clear"].update();markers["volt"].update();markers["one"].update()
...
def on_mouse_press(x, y, button, modifiers):
    ...
    for tile in tiles:
        if C.isMouseOver(x, y, tile.x, tile.y, tile.width, tile.height) and not isGameOver and not tile.hasBeenClicked:
            if markerStatus == "clear": tileClicked(tile)
            elif markerStatus == "volt":
                tile.sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker.png")
                tile.mark = "volt"
            elif markerStatus == "one":
                tile.sprite.image = pyglet.image.load("Assets/Images/One Marker.png")
                tile.mark = "one"

            C.setCursor(window, window.CURSOR_DEFAULT)
    
    for i in markerList:
        if C.isMouseOver(x, y, markers[i].x, markers[i].y, 50, 50) and not isGameOver:
            markerStatus = i
            if markerStatus == "clear": 
                markers["clear"].sprite.image = pyglet.image.load("Assets/Images/Tile Flipped Down - Marker Hover.png") 
            else:
                markers["clear"].sprite.image = pyglet.image.load("Assets/Images/Tile Flipped Down.png")
            
            if markerStatus == "volt":
                markers["volt"].sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker - Marker Hover.png")
            else:
                markers["volt"].sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker.png")
            
            if markerStatus == "one":
                markers["one"].sprite.image = pyglet.image.load("Assets/Images/One Marker - Marker Hover.png")
            else:
                markers["one"].sprite.image = pyglet.image.load("Assets/Images/One Marker.png")
        ...
        if not tiles[i].hasBeenClicked and not isGameOver:
            if C.isMouseOver(x, y, tiles[i].x, tiles[i].y, tiles[i].width, tiles[i].height):
                if not tiles[i].hasBeenHovered:
                    tiles[i].sprite.image = tiles[i].getHoverImage(markerStatus)
                    C.setCursor(window, window.CURSOR_HAND)
                tiles[i].hasBeenHovered = True
            else:
                if tiles[i].hasBeenHovered:
                    tiles[i].sprite.image = tiles[i].getOffHoverImage()
                    C.setCursor(window, window.CURSOR_DEFAULT)
                tiles[i].hasBeenHovered = False
    
    for i in markerList:
        if C.isMouseOver(x, y, markers[i].x, markers[i].y, 50, 50):
            if not markers[i].hasBeenHovered:
                if i == "clear": markers[i].sprite.image = pyglet.image.load("Assets/Images/Tile Flipped Down - Hover.png")
                elif i == "volt": markers[i].sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker - Hover.png")
                elif i == "one": markers[i].sprite.image = pyglet.image.load("Assets/Images/One Marker - Hover.png")
                C.setCursor(window, window.CURSOR_HAND)
            markers[i].hasBeenHovered = True
        else:
            if markers[i].hasBeenHovered:
                if i == "clear":
                    if markerStatus == "clear": 
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/Tile Flipped Down - Marker Hover.png") 
                    else:
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/Tile Flipped Down.png")
                elif i == "volt":
                    if markerStatus == "volt":
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker - Marker Hover.png")
                    else:
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/Voltorb Marker.png")
                elif i == "one": 
                    if markerStatus == "one":
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/One Marker - Marker Hover.png")
                    else:
                        markers[i].sprite.image = pyglet.image.load("Assets/Images/One Marker.png")
                C.setCursor(window, window.CURSOR_DEFAULT)
            markers[i].hasBeenHovered = False
...
def resetBoard(newLevel = level+1, win=True):
    ...
    for i in range(5):
        for k in range(5):
            gameboard[i][k] = "NA"
            if random.randint(1, 20 - 2*level) <= 5: gameboard[i][k] = "V" # 1 in 4 chance of Voltorb
            if random.randint(1, 50 - level) <= 3: gameboard[i][k] = 3   # 1 in 15 chance of '3'
            if random.randint(1, 25 - level) <= 3: gameboard[i][k] = 2   # < 1 in 10 chance of '2'
            if gameboard[i][k] == "NA": gameboard[i][k] = 1     # else, place a '1'
...
print("Initializing Voltorb Flip Version", VERSION)
print("\nTo check for updates, go to 'https://github.com/bmanwell15/Voltorb-Flip.git'")
pyglet.app.run()